...
export default function RegisterPage() {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true)
    try {
      const data = await authApi.register(form.email, form.password, form.display_name)
      login(data.user, data.access_token)
      toast.success('Account created! Welcome to CreatorSync.')
      navigate('/dashboard')
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">Join thousands of creators managing their content.</p>
        </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 ${errors.display_name ? 'border-red-400' : 'border-gray-300'}`}
              />
